...
var xs = [-7.87951290075215, -23.251364738213493, -5.34679518882793, -3.163770449770572,
...
var gaussianScore = function (params, x) {
  var mu = params[0];
  var sigma = params[1];
  return -0.5 * (LOG_2PI + 2 * Math.log(sigma) + (x - mu) * (x - mu) / (sigma * sigma));
}

var gaussFactor = function(ctx, x, params) {
	ctx.lp += gaussianScore(params, x);
};
...
var model_rec = function(ctx, data) {
	...
	var params = repeat_n(nComponents, function(i) {
        ...
        ctx["m_" + i] = m
        var s = (ctx["s_" + i] !== undefined) ? ctx["s_" + i] : ppl.uniform_sample(0, 5)
        ctx["s_" + i] = s
		return [m, s];
	});
	foreach(data, function(i, x) {
		var compi = (ctx["z_" + i] !== undefined) ? ctx["z_" + i] : ppl.random_integer_sample(nComponents);
        ctx["z_" + i] = compi;
		var p = params[compi];
		gaussFactor(ctx, x, p);
	});
...
// console.time('SMC')
// ppl.smc(model_data_annealed, 100, xs.length, true)
// console.timeEnd('SMC')