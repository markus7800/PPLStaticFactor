...
pyro.set_rng_seed(0)
def p(x, y):
    return dist.Normal(1.,1.).log_prob(x) + dist.Normal(x, 0.5).log_prob(y) + dist.Normal(y, 0.5).log_prob(torch.tensor(1.))
...
surrogate_elbo = p(x,y) + (p(x,y) - torch_item(qx.log_prob(x) + qy.log_prob(y))) * (qx.log_prob(x) + qy.log_prob(y))
