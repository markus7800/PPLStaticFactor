...
def model():
    ...
    w = pyro.sample("w", dist.Dirichlet(torch.full((num_clusters,), δ)))

    k = 1
    means = []
    vars = []
    while k <= num_clusters:
        mu = pyro.sample(f"mu_{k}", dist.Normal(ξ, 1/math.sqrt(κ)))
        var = pyro.sample(f"var_{k}", dist.InverseGamma(α, β))
        means.append(mu)
        vars.append(var)
        k = k + 1
    means = torch.hstack(means)
    vars = torch.hstack(vars)

    i = 0
    while i < len(ys):
        z = pyro.sample(f"z_{i}", dist.Categorical(w))
        pyro.sample(f"y_{i}", dist.Normal(means[z], vars[z]), obs=ys[i])
        i = i + 1
...
def guide():
    ...
    continuous_vd("w", num_clusters-1, transforms.StickBreakingTransform())

    k = 1
    while k <= num_clusters:
        mu = continuous_vd(f"mu_{k}", 1)
        var = continuous_vd(f"var_{k}", 1, transforms.ExpTransform())
        k = k + 1
