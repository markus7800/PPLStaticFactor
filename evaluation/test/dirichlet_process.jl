...
@model function dp(ctx::SampleContext, xs::Vector{Float64})
    ...
    alpha::Float64 = sample(ctx, "alpha", InverseGamma(1.,1.))
    i::Int = 0
    while stick > 0.01
        i = i + 1
    ...
    sigma::Float64 = sample(ctx, "sigma", InverseGamma(1.,1.))
    j::Int = 1
    while j <= length(xs)
        z::Int = sample(ctx, "z_" * string(j), Categorical(weights / sum(weights)))
        z = min(z, length(thetas))
        sample(ctx, "x_" * string(j), Normal(get_n(thetas, z), sigma); observed=get_n(xs,j))
        j = j + 1
    end
end
