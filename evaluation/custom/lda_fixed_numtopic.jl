...
function lda_phi_custom(ctx::AbstractManualVisitContext, M::Int, N::Int, V::Int, w::Vector{Int}, doc::Vector{Int}, addr::String)

    old_value::Vector{Float64} = manual_read(ctx, addr)
    new_value::Vector{Float64} = manual_visit(ctx, addr, Dirichlet(fill(1/V,V)))
...
function lda_theta_custom(ctx::AbstractManualVisitContext, M::Int, N::Int, V::Int, w::Vector{Int}, doc::Vector{Int}, addr::String)
    K::Int = 2 # num topics

    old_value::Vector{Float64} = manual_read(ctx, addr)
    new_value::Vector{Float64} = manual_visit(ctx, addr, Dirichlet(fill(1/K, K)))
...
function lda_z_custom(ctx::AbstractManualVisitContext, M::Int, N::Int, V::Int, w::Vector{Int}, doc::Vector{Int}, addr::String)
    n = parse(Int, addr[3:end])
    theta::Vector{Float64} = manual_read(ctx, "theta_" * string(doc[n]))
    old_value::Int = manual_read(ctx, addr)
    new_value::Int = manual_visit(ctx, addr, Categorical(theta))
...
function custom_factor(ctx::AbstractManualVisitContext)
