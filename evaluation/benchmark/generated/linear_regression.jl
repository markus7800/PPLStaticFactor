...
function lr(ctx::AbstractSampleRecordStateContext, xs::Vector{Float64}, ys::Vector{Float64}, _s_::State)
    ...
    while (_s_.i <= length(xs))
        _ = sample_record_state(ctx, _s_, 91, ("y_" * string(_s_.i)), Normal(((_s_.slope * get_n(xs, _s_.i)) + _s_.intercept), 1.0), observed = get_n(ys, _s_.i))
        _s_.i = (_s_.i + 1)
    end
end
...
function lr_intercept_65(ctx::AbstractFactorResampleContext, xs::Vector{Float64}, ys::Vector{Float64}, _s_::State)
    ...
    _s_.i = 1
    while (_s_.i <= length(xs))
        score(ctx, _s_, 91, ("y_" * string(_s_.i)), Normal(((_s_.slope * get_n(xs, _s_.i)) + _s_.intercept), 1.0), observed = get_n(ys, _s_.i))
        _s_.i = (_s_.i + 1)
    end
end
