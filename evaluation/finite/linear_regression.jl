...
function lr_intercept(ctx::AbstractManualVisitContext, xs::Vector{Float64}, ys::Vector{Float64})
    old_value::Float64 = manual_read(ctx, "intercept")
    new_value::Float64 = manual_visit(ctx, "intercept", Normal(0.0, 3.0))
...
function lr_slope(ctx::AbstractManualVisitContext, xs::Vector{Float64}, ys::Vector{Float64})
    old_value::Float64 = manual_read(ctx, "slope")
    new_value::Float64 = manual_visit(ctx, "slope", Normal(0.0, 3.0))
