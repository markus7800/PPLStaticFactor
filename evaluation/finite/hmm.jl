...
function hmm_initial_state(ctx::AbstractManualVisitContext, ys::Vector{Float64}, addr::String)
    ...
    new_value::Int = manual_visit(ctx, addr, Categorical([0.33, 0.33, 0.34]))
...
function hmm_state(ctx::AbstractManualVisitContext, ys::Vector{Float64}, addr::String)
    seqlen::Int = length(ys)
    transition_probs::Matrix{Float64} = [0.1 0.2 0.7; 0.1 0.8 0.1; 0.3 0.3 0.4]
    ...
    old_value::Int = manual_read(ctx, addr)
    new_value::Int = manual_visit(ctx, addr, Categorical(get_row(transition_probs, current)))
