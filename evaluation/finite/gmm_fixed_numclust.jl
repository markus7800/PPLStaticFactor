...
function gmm_mu(ctx::AbstractManualResampleContext, ys::Vector{Float64}, addr::String)
    ξ::Float64 = 0.0
    κ::Float64 = 0.01

    old_value::Float64 = manual_read(ctx, addr)
    new_value::Float64 = manual_resample(ctx, addr, Normal(ξ, (1 / sqrt(κ))))
...
function gmm_var(ctx::AbstractManualResampleContext, ys::Vector{Float64}, addr::String)
    α::Float64 = 2.0
    β::Float64 = 10.0
...
function gmm_w(ctx::AbstractManualResampleContext, ys::Vector{Float64}, addr::String)
    δ::Float64 = 5.0
    num_clusters::Int = 4

    old_value::Vector{Float64} = manual_read(ctx, addr)
    new_value::Vector{Float64} = manual_resample(ctx, addr, Dirichlet(fill(δ, num_clusters)))
...
function gmm_z(ctx::AbstractManualResampleContext, ys::Vector{Float64}, addr::String)
    w::Vector{Float64} = manual_read(ctx, "w")

    i::Int = parse(Int, addr[3:end])

    old_value::Int = manual_read(ctx, addr)
    new_value::Int = manual_resample(ctx, addr, Categorical(w))
